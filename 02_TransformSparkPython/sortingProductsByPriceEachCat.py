...
for i in productsGroupBy.map(lambda rec: sorted(rec[1], key=lambda k: float(k.split(",")[4] if (k.split(",")[4] != "" and not " " in k.split(",")[4]) else "12345.67"))).takeOrdered(5, lambda x: -float(x.split(",")[4])):
	print(i)
